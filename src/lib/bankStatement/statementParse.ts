// statementParse.ts
// Turns raw statement text into ParsedRow[]. Pure functions, no I/O, no DB.
//
// Two adapters ship here:
//   parseMixxYas  - the Tigo Pesa / Mixx by Yas PDF text layout
//   parseDelimited - a generic CSV/TSV adapter for CRDB, NMB, Equity exports
//
// Ask the bank for CSV wherever possible. PDF text extraction is the most
// fragile part of this pipeline and the reason every import is balance-checked
// before it is allowed to post anything.
 ...
 *
 * A record starts at a dd/mm/yyyy token and runs until the next one. Inside it
 * we look for the labelled fields, then take the last three bare numbers as
 * (money out, money in, balance) because that is the column order the PDF
 * renders. Header and footer noise is skipped by requiring a TxnID.
 */
export function parseMixxYas(text: string): ParsedRow[] {
  const cleaned = text.replace(/\r/g, '');
  const tokens = cleaned.split(/(?=\b\d{2}\/\d{2}\/\d{4}\b)/g);
  ...
  for (const chunk of tokens) {
    ...
    // strip the labelled numbers so they cannot be mistaken for column values
    const bare = chunk
...
/** Generic CSV/TSV adapter for bank exports. Header names are matched case-insensitively. */
...
/** Stable fingerprint of the file, so the same statement cannot import twice. */
