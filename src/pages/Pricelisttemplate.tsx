...
const PricelistDocument = ({ products, settings, selectedCats, listTitle }: {
  ...
  return (
    <div id="malkia-pricelist" style={{ width: 680, background: '#fff', fontFamily: "'Instrument Sans', sans-serif", color: '#1a1a1a', fontSize: 12 }}>
      {/* Header */}
      <div style={{ padding: '28px 36px 20px', borderBottom: `3px solid ${p}` }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <img src={settings.logo_url || MALKIA_LOGO} alt="Malkia" style={{ height: 90, width: 'auto', objectFit: 'contain', flexShrink: 0 }} />
            <div>
...
export default function PricelistTemplate() {
  ...
  const printPricelist = () => {
    const el = document.getElementById('malkia-pricelist')
    if (!el) return
    const printRes = printHtmlDocument(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Price List — ${listTitle}</title>
            ...
            <div className="card">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 70, height: 70, border: '2px dashed var(--border)', borderRadius: 10, overflow: 'hidden', background: 'var(--surface2)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <img src={settings.logo_url || MALKIA_LOGO} alt="Logo" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                </div>
                <div>
