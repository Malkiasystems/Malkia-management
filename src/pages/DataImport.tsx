...
function getTemplateCols(entity: ImportEntity, locations: StockLocation[]): TemplateCol[] {
  if (entity === 'products') {
    ...
    return [
      ...
      { header: 'Brand',                ex1: 'Spectra',                  ex2: 'Malkia',                  ex3: 'Generic',                 note: 'Optional. Product brand name.' },
...
function downloadTemplate(entity: ImportEntity, locations: StockLocation[]) {
  ...
  const rows = [
    `"MalkiaOS Import Template — ${labels[entity]}"`,
  ...
  a.href = url
  a.download = `MalkiaOS_${labels[entity]}_Template.csv`
  a.click()
...
function parseCSV(text: string): ParsedRow[] {
  ...
  // Skip instruction/comment rows (start with quotes containing [ or MalkiaOS)
  const lines = allLines.filter(l => {
    const t = l.trim()
    return t && !t.startsWith('"MalkiaOS') && !t.startsWith('"INSTRUCTIONS') && !t.match(/^"?\[/)
  })
...
const SOURCES: { id: ImportSource; label: string; ext: string; desc: string; badge?: string }[] = [
  ...
  { id: 'excel_csv',  label: 'Excel / CSV', ext: '.csv,.xlsx', desc: 'Any spreadsheet. Download the MalkiaOS template for the exact format.' },
...
export default function DataImport() {
  ...
  return (
    <div style={s.page}>
      ...
      <p style={s.sub}>Migrate data from Tally, QuickBooks, Excel, or any spreadsheet. Download a template to see the exact format expected.</p>
      <div style={s.stepper}>
