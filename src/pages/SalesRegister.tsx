...
export default function SalesRegister() {
  ...
  const loadSales = async () => {
    ...
    const { data, error } = await supabase
      .from('vouchers')
      .select('ref, description, total_amount, vat_amount, subtotal, payment_method, posting_date, status, customers(name, whatsapp)')
      .eq('type', 'cash_sale')
  ...
  const totalVat = sales.reduce((s, v) => s + (v.vat_amount || 0), 0)
