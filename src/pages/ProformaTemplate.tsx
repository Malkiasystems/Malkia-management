...
// ═══════════════════════════════════════════════════════════════════════════
//  MALKIA PROFORMA INVOICE TEMPLATE
...
export const DEFAULT_PROFORMA: ProformaSettings = {
  ...
  footer_tagline: 'Kila mama ni Malkia',
  footer_thank_you: 'Thank you for considering Malkia Wellness. We look forward to partnering with you.',
  footer_website_cta: 'malkia.co.tz · Reimagining Motherhood',
  terms_conditions: 'Prices are valid until the date shown above. Delivery within Dar es Salaam is free for orders above 500,000 TZS. Goods remain property of Malkia Wellness Group Ltd until full payment is received. VAT is included where applicable.',
...
const qrTarget = (s: ProformaSettings, ref: string, total: number): string => {
  switch (s.qr_mode) {
    ...
    case 'whatsapp':
      return `https://wa.me/${s.qr_whatsapp_number}?text=${encodeURIComponent(`Hi Malkia, I'd like to accept proforma ${ref}`)}`
    case 'website':
...
// ═══════════════════════════════════════════════════════════════════════════
//  MalkiaProforma — the actual printable document
// ═══════════════════════════════════════════════════════════════════════════
export function MalkiaProforma({ voucher, settings }: {
...
export function ProformaTemplateSettings({ settings, onChange }: {
  ...
  return (
    <div>
      ...
      {tab === 'theme' && (
        <div>
          ...
          <div style={{ marginTop: 16 }}>
            <div style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>Malkia Presets</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {[
        ...
        <div>
          ...
          {settings.qr_enabled && (
            <>
              ...
              {settings.qr_mode === 'accept'   && <Fld label="Accept URL Base"    k="accept_url_base"  settings={settings} onChange={set} placeholder="https://malkia.co.tz/accept" />}
              {settings.qr_mode === 'konnect'  && <Fld label="Konnect URL"        k="qr_konnect_url"   settings={settings} onChange={set} placeholder="https://www.malkia.co.tz/join" />}
      ...
      {tab === 'footer' && (
        <div>
          <Fld label="Tagline (bold bottom-left)"   k="footer_tagline"     settings={settings} onChange={set} placeholder="Kila mama ni Malkia" />
          <Fld label="Thank-you message"            k="footer_thank_you"   settings={settings} onChange={set} placeholder="Thank you for considering Malkia..." textarea />
          <Fld label="Website / CTA line"           k="footer_website_cta" settings={settings} onChange={set} placeholder="malkia.co.tz · Reimagining Motherhood" />
...
export default function ProformaTemplatePage() {
  ...
  const resetToDefault = () => {
    if (confirm('Reset all proforma settings to Malkia defaults?')) {
  ...
  const SAMPLE: ProformaVoucher = {
    ...
    voucher_lines: [
      { qty: 10, unit_price: 32000, discount_pct: 0, total: 320000, description: 'Nipple Cream', products: { name: 'Malkia Nipple Cream — 60ml', sku: 'MK-003' } },
      { qty: 4,  unit_price: 65000, discount_pct: 10, total: 234000, description: 'Prenatal Kit', products: { name: 'Malkia Prenatal Bundle — Essentials', sku: 'MK-008' } },
    ],
  }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '360px 1fr', gap: 24, alignItems: 'start' }}>
        ...
        <div>
          ...
          <div style={{ transform: 'scale(0.62)', transformOrigin: 'top left', width: '162%' }}>
            <MalkiaProforma voucher={SAMPLE} settings={settings} />
          </div>
        </div>
      </div>
    </div>
